...
<head>
  ...
  <c:import url="/WEB-INF/views/common/head-4a.jsp">
    <c:param name="title" value="${entity.estName}" />
    <c:param name="desc" value="${entity.estName} - ${entity.teaser}" />
    <c:param name="kw" value="${entity.estName}, best ${entity.category} in ${city}" />
  </c:import>
</head>
<!-- Body-->
<body>
  ...
  <section class="container pt-5 mt-5">
    ...
    <nav aria-label="breadcrumb">
      <ol class="pt-lg-3 pb-lg-4 pb-2 breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">${entity.estName}</li>
      </ol>
    </nav>
    <div class="row">
      <div class="col-lg-9 col-xl-8 pe-lg-4 pe-xl-0">
        <!-- Post title + post meta-->
        <h1 class="pb-2 pb-lg-3">${entity.estName}</h1>
        ...
        <c:if test="${not empty entity.image1}">
          <figure class="figure"><img class="figure-img rounded-5 mb-3" src="${entity.image1}" alt="${entity.estName}">
            <figcaption class="figure-caption">Image: ${entity.estName}</figcaption>
          </figure>
        </c:if>
        ...
        <c:if test="${entity.category eq 'hospital'}">
          <h1 class="pb-3 pb-lg-4">Doctors of ${entity.estName}</h1>
          <!-- Post-->
        ...
        <c:forEach var="ent" items="${relEntities}">
          ...
          <article class="swiper-slide">
            <div class="position-relative"><img class="rounded-5" src="${ent.image1}" alt="Post image">
              <h3 class="h4 mt-4 mb-0"><a class="stretched-link" href="${ent.url}-${ent.id}--sb">${ent.estName}</a></h3>
            </div>
          </article>
        </c:forEach>
      </div>
